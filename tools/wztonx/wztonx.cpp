...
struct wztonx {
    ...
    std::string convert_str(std::u16string const & p_str) {
        std::string result;
        result.reserve(p_str.size() * 3);
        for (char16_t ch : p_str) {
            if (ch < 0x80) {
    ...
    void write_bitmaps() {
        ...
        for (auto index = 0u; index < bitmaps.size(); ++index) {
            ...
            if (check != pixels * 4) {
                std::cerr << "Size mismatch: " << std::dec << width << "," << height << "," << decompressed << "," << f1 << "," << f2 << " (ignoring)" << std::endl;
                // Fill with transparent black and skip format conversion
                std::fill(input.begin(), input.begin() + size, 0);
                goto write_bitmap;
            }
            switch (f1) {
            case 1:
            ...
            if (hc) {
                final_size = static_cast<uint32_t>(
                    LZ4_compress_HC(reinterpret_cast<char const *>(input.data()),
                        reinterpret_cast<char *>(output.data()), size, static_cast<int>(output.size()), LZ4HC_CLEVEL_DEFAULT));
            } else {
                final_size = static_cast<uint32_t>(
